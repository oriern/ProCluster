...
class BinaryDiceLoss(nn.Module):
    ...
    def forward(self, predict, target):
        ...
        # num = 2 * torch.sum(torch.mul(torch.mul(1-predict,predict), target), dim=0) + self.smooth
        # den = torch.sum(torch.mul(1-predict,predict).pow(self.p) + target.pow(self.p), dim=0) + self.smooth

        num = 2 * torch.mul(predict, target) + self.smooth
        den = predict.pow(self.p) + target.pow(self.p) + self.smooth

        # num = 2 * torch.mul(torch.mul(1-predict,predict), target) + self.smooth
        # den = torch.mul(1-predict,predict).pow(self.p) + target.pow(self.p) + self.smooth
...
class LongformerForSequenceClassificationRanking(LongformerPreTrainedModel):
    ...
    def forward(
        ...
        higher_ranked = self.tanh(logits)





        return_dict2 = return_dict2 if return_dict2 is not None else self.config.use_return_dict

        if global_attention_mask2 is None:
            logger.info("Initializing global attention on CLS token...")
            global_attention_mask2 = torch.zeros_like(input_ids2)
            # global attention on cls token
        ...
        outputs2 = self.longformer(
            ...
            global_attention_mask=global_attention_mask2,
        ...
        span_mask2 = global_attention_mask2[:]
        span_mask2[0] = 0
        logits2 = self.classifier(sequence_output2, span_mask=span_mask2)


        lower_ranked = self.tanh(logits2)

        loss = 0
        for i in range(len(input_ids)):


            loss_fct = MarginRankingLoss(self.margin*(ranking2[i]-ranking1[i]))
            assert(ranking2[i]-ranking1[i] >= 0)
            # ones = torch.ones_like(input_ids)
            loss += loss_fct(higher_ranked.view(-1), lower_ranked.view(-1), labels.view(-1))
    ...
    def forward(
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                # kwargs = {"alpha": 0.25, "gamma": 2.0, "reduction": 'mean'}
                # loss_fct = BinaryFocalLossWithLogits(**kwargs)
                # loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))

                # loss_f1 = BinaryDiceLoss()
                # # loss = 0.5 * loss_fct(logits.view(-1, self.num_labels), labels.view(-1)) + 0.5 * loss_f1(logits.view(-1, self.num_labels), labels.view(-1))
                # loss = loss_f1(logits.view(-1, self.num_labels), labels.view(-1))
